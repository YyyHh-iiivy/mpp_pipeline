...
static void thread1_entry(void *parameter)
{
    ...
    while (1)
    {
        ...
        {
            count++;  // 释放信号量时计数增加
            rt_kprintf("rt_sem_release: count=%d\n", count);
        }
...
static void thread2_entry(void *parameter)
{
    ...
    while (1)
    {
        ...
        {
            count--;  // 获取信号量时计数减少
            rt_kprintf("rt_sem_take: count=%d\n", count);
        }
        else
        {
            rt_kprintf("rt_sem_take: error\n");
...
int main(void)
{
    ...
    /* 创建动态线程thread1，优先级为 THREAD_PRIORITY = 15 */
    ...
    /* 创建动态线程thread2，优先级为 THREAD_PRIORITY = 15 */
