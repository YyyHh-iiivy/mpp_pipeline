...
运行期间每5秒（仅在已有样本时）输出一次延迟画像窗口，覆盖DATAFIFO消息年龄、复制、READ_DONE、RTP发送以及UDP发送队列前后值：

```text
[latency:profile] count=... msg_age_avg_ms=... msg_age_p90_ms=... msg_age_max_ms=... copy_avg_ms=... read_done_avg_ms=... send_avg_ms=... outq_before_avg=... outq_after_avg=...
```

该摘要只用于区分发送链与播放器缓存的延迟来源，不改变RTP发送、丢帧或自然随机访问恢复策略。
